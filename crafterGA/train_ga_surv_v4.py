...
ACHIEVEMENT_TIERS = {
    ...
    "place_table": 5.0,
    "make_wood_pickaxe": 6.0,
    "make_wood_sword": 5.0,

    "place_furnace": 7.0,
    "make_stone_pickaxe": 8.0,
    "make_stone_sword": 7.0,
    "defeat_zombie": 6.0,
    "defeat_skeleton": 6.0,
    "eat_cow": 4.0,
...
def fitness_func(ga_instance, solution, sol_idx):
    ...
    try:
        visited_positions.add(tuple(env.unwrapped._player.pos))
    ...
    while not done:
        ...
        current_pos_tuple = tuple(info['player_pos'])
        if current_pos_tuple not in visited_positions:
            custom_reward += EXPLORATION_BONUS
            visited_positions.add(current_pos_tuple)

        current_achievements = {ach for ach, count in info['achievements'].items() if count > 0}
        newly_unlocked = current_achievements - unlocked_achievements
        
        if newly_unlocked:
            for ach in newly_unlocked:
                bonus = ACHIEVEMENT_TIERS.get(ach, 1.0)
                custom_reward += bonus
        
        unlocked_achievements = current_achievements
...
if __name__ == "__main__":
    ...
    parser.add_argument('--outdir', default='logdir/crafter_ga_imp3/final')
